...
| 床摩擦係数 | `FLOOR_FRICTION` | 0.9 | 床（キャンバス下端）の摩擦（無次元） |
| 床反発係数 | `FLOOR_ELASTICITY` | 0.5 | 床の弾性（無次元） |
| 安全柵摩擦係数 | `BOUNDARY_FRICTION` | 0.5 | 上/左/右の仮想壁の摩擦（無次元） |
| 安全柵反発係数 | `BOUNDARY_ELASTICITY` | 0.5 | 上/左/右の仮想壁の弾性（無次元） |
...
> **摩擦係数・反発係数の合成について**
> pymunk (Chipmunk) は接触する2面の値を **掛け算** で合成します。落下したバーが跳ねる大きさを決めるのは
> 「バー × 相手の面」の積であり、既定値では バー↔斜面/壁 = 0.6×0.6 = **0.36**、バー↔床 = 0.6×0.5 = **0.30** です。
> これらの値は Streamlit UI のサイドバー「摩擦係数・反発係数（跳ね返りの強さ）」から面ごとに変更できます。
