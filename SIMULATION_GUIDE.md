...
- 初速度は既定でゼロ（静止状態からの落下）
- 脱着力を再現する場合は `RELEASE_VELOCITY_MPS` [m/s] と
  `RELEASE_VELOCITY_ANGLE_DEG` [°] でリリース時の初速度を与えられる
  （内部単位への換算は上記の重力スケールに合わせて 1 m/s = 100 px/s）
- 初速度の向きの基準は `RELEASE_VELOCITY_ANGLE_MODE` で切り替える
  - `"BAR_NORMAL"`（既定）: バー長手面の法線のうち**下向き側**を 0° とする相対角。
    バーは壁と平行に立つ姿勢で落下するため、この法線は斜面と平行になり、
    絶対角では `180° + ステージ角度`（＋相対リリース角度）になる。
    例) ステージ 45° → 225°、60° → 240°。台座の斜面に応じて向きが自動で回転する
  - `"ABSOLUTE"`: 画面固定軸（0°=+X, 90°=+Z）を基準にする旧仕様
