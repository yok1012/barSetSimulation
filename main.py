...
### --- ヘルパー関数群 ---
def compute_release_pos(angle_deg, x_offset, y_offset):
    """リリース（落下開始）座標を計算する。

    オフセットの原点は「理想位置」（calculate_ideal_position）。
    つまり offset=(0,0) のときリリース位置＝理想位置になり、オフセットは
    「理想位置からのずらし量」を表す。

    ANGLE_LINKED_OFFSET が False の場合は画面固定軸：
        実X = ideal_x + x_offset, 実Y = ideal_y - y_offset
    True の場合はステージ傾斜 θ=radians(-angle_deg) に連動して軸を回転させる：
        x_offset = 斜面方向、y_offset = 壁方向（斜面に垂直）
    """
    ...
    if ANGLE_LINKED_OFFSET:
        ...
        rx = ideal_x + x_offset * cos_t + y_offset * sin_t
        ry = ideal_y + x_offset * sin_t - y_offset * cos_t
        return rx, ry
...
def is_near_ideal_position(bar_position, stage_angle_rad):
    """バー重心が理想位置から IDEAL_NEIGHBORHOOD_RADIUS(μm) 以内にあるか判定する。
    True を返すとき、その短冊接触は「正常な収まり」とみなしカウントから除外する。"""
    ideal_x, ideal_y = calculate_ideal_position(stage_angle_rad)
    dist = math.sqrt((bar_position[0] - ideal_x) ** 2 + (bar_position[1] - ideal_y) ** 2)
    return dist <= IDEAL_NEIGHBORHOOD_RADIUS
...
def draw_arrow(surface, color, start, end, width=5, head=26):
    ...
    for da in (math.radians(150), math.radians(-150)):
        ...
        pygame.draw.line(surface, color, end, (int(hx), int(hy)), width)


def check_bar_contact_side(bar_body, wall_segment, contact_point, contact_normal=None):
    """
    バーのどの面（長手方向か短手方向）が壁に接触しているかを判定する。

    contact_normal が与えられた場合は「接触法線の向き」で判定する（推奨）。
    バーは細長い矩形のため、接触点の位置だけで判定すると角（頂点）での接触が
    long/short の両方の閾値を満たして "both" になりやすい。法線の向きで見れば、
    どの面が壁に向いているかが一意に決まり、短冊（短手面）接触を正しく判定できる。

    contact_normal が None のときは従来の接触点位置ベースの判定にフォールバックする。

    Args:
        bar_body: バーの物理ボディ
        wall_segment: 壁のセグメント（未使用）
        contact_point: 接触点の座標
        contact_normal: 接触法線（Vec2d 等の (x, y) 添字アクセス可能なもの）。任意。
    ...
    if contact_normal is not None:
        # 接触法線をバーのローカル座標系に変換する
        cos_a, sin_a = math.cos(-bar_angle), math.sin(-bar_angle)
        nx, ny = contact_normal[0], contact_normal[1]
        local_nx = nx * cos_a - ny * sin_a   # バー短手(幅)方向の法線成分
        local_ny = nx * sin_a + ny * cos_a   # バー長手(長さ)方向の法線成分
        # 法線が長手方向(local y)に近い → 短手面が壁に向いている = 短冊接触
        # 法線が短手方向(local x)に近い → 長辺面が壁に向いている = 長手接触
        if abs(local_ny) >= abs(local_nx):
            return "short_side"
        return "long_side"

    # --- 従来の接触点位置ベースの判定（後方互換のフォールバック） ---
...
def run_interactive_mode():
    ...
    def wall_contact_handler(arbiter, space, data):
        ...
        if not bar_is_settled and dynamic_bars:
            ...
            if contact_points:
                ...
                # バーのどの面が接触しているかを判定
                contact_side = check_bar_contact_side(dynamic_bars[0].body, None, contact_point, arbiter.contact_point_set.normal)
                ...
                if contact_side in ["short_side", "both"]:
                    ...
                    if is_near_ideal_position(dynamic_bars[0].body.position, stage_angle_rad):
                        print(f"短冊接触(理想位置近傍のため無視): 位置 ({contact_point[0]:.1f}, {contact_point[1]:.1f})")
                        return True
                    # 以前の接触点との距離を計算
                    diff = 0.0
                    if contact_history:
    ...
    stage_short_handler.data["hit_angle"] = None
    stage_short_handler.data["contact_side"] = None
    stage_short_handler.data["contact_point"] = None
    stage_short_handler.data["contact_count"] = 0
    ...
    def reset_simulation(full_reset=True, check_pos=True):
        ...
        for _h in (wall_handler, stage_short_handler):
            _h.data["hit_position"] = None
            _h.data["hit_angle"] = None
            _h.data["contact_side"] = None
            _h.data["contact_point"] = None
            _h.data["contact_count"] = 0
        # 接触回数をリセット
    ...
    while running:
        ...
        if wall_hit_flag[0] and not bar_is_settled:
            # 複数回短冊方向接触の場合（壁・斜面どちらの接触でも）
            if (wall_handler.data.get("contact_side") == "multiple_short_side"
                    or stage_short_handler.data.get("contact_side") == "multiple_short_side"):
...
def run_single_condition_mode():
    ...
    try:
        ...
        font_label = pygame.font.Font(JP_FONT_FILENAME, 72)
        font_label_s = pygame.font.Font(JP_FONT_FILENAME, 56)
    except pygame.error:
        font_coords = pygame.font.Font(None, 20)
        font_label = pygame.font.Font(None, 80)
        font_label_s = pygame.font.Font(None, 64)
    ...
    # --- 短冊接触の集計（診断用）。斜面(STAGE)と壁(WALL)の両方をbeginイベントで分類・カウント ---
    contact_stats = {"wall_begin": 0, "slope_begin": 0, "short": 0, "long": 0, "unknown": 0,
    ...
    def single_contact_handler(arbiter, space, data):
        ...
        contact_stats[surface + "_begin"] += 1
        contact_points = arbiter.contact_point_set.points
        if not contact_points:
            return True
        contact_point = contact_points[0].point_a
        side = check_bar_contact_side(bar.body, None, contact_point, arbiter.contact_point_set.normal)
        if side == "long_side":
            contact_stats["long"] += 1
            return True
        if side != "short_side":
            contact_stats["unknown"] += 1
            return True
        # short_side（短冊面が接触）
        ...
        if contact_history:
            ...
            diff = math.sqrt((contact_point[0] - last_point[0]) ** 2 + (contact_point[1] - last_point[1]) ** 2)
        contact_stats["accumulated_diff"] += diff
        contact_stats["counted_short"] += 1
    ...
    diff_px = math.sqrt((ideal_x - final_x) ** 2 + (ideal_y - final_y) ** 2)
    diff_mm = diff_px / PPM
    ...
    for text, color in lines:
        surface.blit(font_label.render(text, True, color), (box_x + pad, ty))
        ty += 84

    # 短冊接触の集計テキスト（ズレ値ボックスの下）
    c = contact_stats
    contact_lines = [
        f"短冊接触(short_side): {c['short']} 回 （斜面 {c['short_slope']} / 壁 {c['short_wall']}）",
        f"  理想位置近傍で除外: {c['excluded_near_ideal']} / 有効カウント: {c['counted_short']} (累積差分 {c['accumulated_diff']:.2f} μm)",
        f"長手接触(long_side): {c['long']} 回 / 分類不能: {c['unknown']} 回",
        f"接触イベント: 斜面 {c['slope_begin']} / 壁 {c['wall_begin']}",
    ]
    cbox_y = box_y + box_h + 20
    cbg = pygame.Surface((1900, pad * 2 + len(contact_lines) * 64), pygame.SRCALPHA)
    cbg.fill((255, 255, 255, 210))
    surface.blit(cbg, (box_x, cbox_y))
    cty = cbox_y + pad
    ...
    print("--- 理想位置からのズレ ---")
    print(f"理想位置: ({ideal_x:.3f}, {ideal_y:.3f}) μm")
    print(f"最終位置: ({final_x:.3f}, {final_y:.3f}) μm")
    print(f"ズレ: {diff_px:.3f} μm ({diff_mm:.5f} mm)  判定: {'OK' if within_tol else 'NG'} (許容 {tolerance:.0f} μm)")
    ...
    print(f"短冊(short_side)接触: {c['short']} 回（斜面 {c['short_slope']} / 壁 {c['short_wall']}）")
    print(f"  うち理想位置近傍で除外: {c['excluded_near_ideal']}")
    print(f"  有効カウント: {c['counted_short']}（累積差分 {c['accumulated_diff']:.3f} μm）")
    print(f"長手(long_side)接触: {c['long']} 回 / 分類不能: {c['unknown']} 回")
    print(f"SHORT_TOTAL: {c['short']}")
    print(f"SHORT_SLOPE: {c['short_slope']}")
    print(f"SHORT_WALL: {c['short_wall']}")
    print(f"SHORT_COUNTED: {c['counted_short']}")
...
def run_single_condition_parallel(params_data):
    ...
    for trial_num in trial_range:
        try:
            ...
            release_pos_x, release_pos_y = compute_release_pos(angle_deg, rx_offset + random_x_offset, ry_offset - random_y_offset)
    ...
    for i, params_tuple in enumerate(param_combinations):
        ...
        for trial_num in range(NUM_TRIALS_PER_CONDITION):
            ...
            def wall_contact_handler(arbiter, space, data):
                ...
                if contact_points:
                    contact_point = contact_points[0].point_a
                    contact_side = check_bar_contact_side(data["bar_body"], None, contact_point, arbiter.contact_point_set.normal)
