...
On Windows (and any `spawn` start-method), parallel worker processes **re-import `main.py` fresh**, so the globals `sim_runner` patched in the parent are lost. Variability (`RELEASE_*_VARIABILITY`), `SIMULATION_DURATION`, contact thresholds, and floor-fail validation fall back to `main.py`'s file defaults in workers. Only the param ranges, trial count, and the globals listed in `WORKER_INHERITED_GLOBALS` survive — those are bundled by `collect_worker_settings()` into each task tuple and restored by `apply_worker_settings()` at the top of `run_single_condition_parallel()`. That list currently covers the release-velocity settings (`RELEASE_VELOCITY_MPS` / `RELEASE_VELOCITY_ANGLE_DEG` / `VELOCITY_MPS_TO_SIM`) and the friction/elasticity coefficients (`BAR_*` / `WALL_*` / `FLOOR_*` / `BOUNDARY_*`). To make another setting survive, add its name to `WORKER_INHERITED_GLOBALS`; otherwise use sequential `BATCH` or change the default in `main.py` itself.
